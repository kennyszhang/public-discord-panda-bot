...
bot.on('message', message => {
    ...
    switch(args[0]) {
        ...
        case 'feed':
            ...
            var date = new Date();
            message.react('❤️');
            var images = ['./images/panda.png', './images/panda2.png', './images/panda3.jpg', './images/panda4.jpg'];
            var num = Math.floor(Math.random() * (images.length));
            const attachment = new MessageAttachment(images[num]);
            MongoClient.connect(url, {useNewUrlParser: true, useUnifiedTopology: true}, function(err, db) {
                if (err) throw err;
                var dbo = db.db("discord-panda-bot");
                dbo.collection("feed-counter").find({}).toArray(function(err, result) {
                    if (err) throw err;
                    counter = result.length;
                    message.channel.send('```thank you kind stranger!\nso far i have been fed ' + counter + ' times\nhave a great day!```');
                    ...
                    dbo.collection("feed-counter").insertOne(obj, function(err, res) {
                        if (err) throw err;
                        db.close();
                    });
                });
